...
function Edit() {
  ...
  useEffect(() => {
    ...
    // Extract the logged-in user's name from the JWT token
    const token = localStorage.getItem("token");
    if (token) {
      try {
        const decoded = jwtDecode(token);
        setData(prevData => ({ ...prevData, owner: decoded.username })); // Set owner automatically
      } catch (error) {
  ...
  function handleSubmit(e) {
    ...
    console.log("Sending update request for:", projects_name); // Debugging
